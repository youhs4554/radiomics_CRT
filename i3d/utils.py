...
__all__ = ["generate_backbone", "freeze_layers",
           "init_layers", "load_pretrained_ckpt"]
...
def inflate_linear(linear2d, time_dim):
    ...
    """
    linear3d = torch.nn.Linear(
        linear2d.in_features * time_dim, linear2d.out_features)
...
def inflate_pool(pool2d, time_dim=1, time_padding=0, time_stride=None, time_dilation=1):
    if isinstance(pool2d, torch.nn.AdaptiveAvgPool2d):
        ...
        if isinstance(pool2d, torch.nn.MaxPool2d):
            # use adaptive maxpooling for reproducibility
            pool3d = torch.nn.AdaptiveMaxPool3d((39, 56, 56))
